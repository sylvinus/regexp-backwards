...
func compile(expr string, mode syntax.Flags, longest bool) (*Regexp, error) {
	...
	// If the pattern is anchored at the end only, prefer scanning backwards.
	if re.Op == syntax.OpConcat && len(re.Sub) > 0 && re.Sub[len(re.Sub)-1].Op == syntax.OpEndText && re.Sub[0].Op != syntax.OpBeginText {
		reverseRegexp(re)
		re = re.Simplify()
